...
export async function jsonTransArrProcess(
  data: unknown,
  jsonPath: PathOrFileDescriptor,
  fromLang: Lang,
  toLang: Lang[]
) {
  const pathsArr: PathOrFileDescriptor[] = [];
  for (let k in toLang) {
    pathsArr.push(await jsonTransProcess(data, jsonPath, fromLang, toLang[k]));
  }
  return pathsArr;
}

export async function jsonTransProcess(
  data: unknown,
  jsonPath: PathOrFileDescriptor,
  fromLang: Lang,
  toLang: Lang
): Promise<PathOrFileDescriptor> {
  let translatedJson = Array.isArray(data)
    ? await transArray(data, fromLang, toLang)
    : isObject(data) && (await transObject(data, fromLang, toLang));
